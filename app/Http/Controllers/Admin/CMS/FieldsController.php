...
class FieldsController extends Controller
{
    ...
    public function __construct()
    {
        ...
        $this->selectCategory = (new FieldsCategory())->getCategorySelect();
    ...
    public function edit($id, Request $request)
    {
        ...
        $item = $this->baseModel->with('categories')->find($id);
        $categoryIdArr = $item->categories->pluck('id');

        return view($this->route.'.form', compact('item'))
            ->with('selectCategory', $this->selectCategory)
            ->with('categoryIdArr', $categoryIdArr)
            ->with('route', $this->route);
    }
    ...
    public function store(Request $request)
    {
        ...
        $arr_categories = $data['category_id'];
        $data['category_id'] = array_shift($arr_categories);
    ...
    public function update($id, Request $request)
    {
        ...
        $data['text_plus'] = $textPlus;

        $updated = tap($this->baseModel
            ->find($id))->update($data);

        $updated->categories()->sync($request->category_id);
