 ...
 * GoogleAuthenticator
 *
 * GoogleAuthenticator for app
 * php version 7.0
 *
 * @category   StatusCommunication
 * @package    App\Libraries
 * @subpackage GoogleAuthenticator
...
class GoogleAuthenticator
{
     ...
     *
     * @param int $secretLength Secret length
     *
     * @return string
     */
     ...
     * @param int|null $timeSlice Time slice in seconds
     *
     * @return string
     */
    ...
    public function getQRCodeGoogleUrl(
        ...
        $level = !empty($params['level'])
            && array_search($params['level'], array('L', 'M', 'Q', 'H')) !== false
            ? $params['level'] : 'M';
        ...
        return "https://api.qrserver.com/v1/create-qr-code/"
            ."?data=$urlencoded&size=${width}x${height}&ecc=$level";
    }

    /**
     * Check if the code is correct.
     * This will accept codes starting from $discrepancy*30sec ago
     * to $discrepancy*30sec from now.
     *
     * @param string   $secret           Secret code
     * @param string   $code             Verify code
     * @param int      $discrepancy      This is the allowed time drift in
     *                                   30 second units
     *                                   (8 means 4 minutes before or after)
        ...
        for ($i = 0; $i < 4; ++$i) {
            if ($paddingCharCount == $allowedValues[$i]
                && substr(
                    $secret,
                    -($allowedValues[$i])
                ) != str_repeat($base32chars[32], $allowedValues[$i])
            ) {
                return false;
            }
        }
     ...
     */
    private function _timingSafeEquals($safeString, $userString)
    {
