...
<div class="kt-subheader   kt-grid__item" id="kt_subheader">
    <div class="kt-container  kt-container--fluid ">
        ...
        <div class="kt-subheader__toolbar">
            ...
            @endcan
            @can('permission', $route.'.create')
                <a href="{{ route($route.'.create', ['position' => request()->input('type') ?? $type]) }}" class="btn btn-brand btn-elevate btn-icon-sm">
                    <i class="la la-plus"></i>
                    @lang('admin.common.add')
                </a>
            @endcan
                ...
                <div class="kt-portlet__body">
                    <form class="kt-form kt-form--fit filterForm" name="filterForm" method="get">
                        ...
                        <div class="row kt-margin-b-20">
                            <div class="col-md-4 kt-margin-b-10-tablet-and-mobile d-none">
                                <label>Type:</label>
                            ...
                            <div class="col-md-4 kt-margin-b-10-tablet-and-mobile">
                                ...
                                <select class="form-control kt-input" name="type" data-col-index="2">
                                    @if (isset($bannersPosition) && count($bannersPosition))
                                        @foreach ($bannersPosition as $key => $value)
                                            <option value="{{ $key }}" {{ request('type') && request('type') == $key ? 'selected' : '' }}>{{ $value['text'] . '  (' . $value['width'] . ' x ' . $value['height'] . ')' }} </option>
                                        @endforeach
                                    @endif
                                </select>
                            </div>
