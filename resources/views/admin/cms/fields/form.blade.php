                ...
                {!! Form::open( ['url' => ($item->id ? route($route).'/'.$item->id : route($route)), 'method' => ($item->id ? 'PATCH' : 'POST'), 'class' => 'kt-form kt-form--label-right formData', 'name'=>'formData', 'files'=>true] ) !!}
                    ...
                    <div class="kt-portlet__body">
                        <div class="form-group row">
                            ...
                            <div class="col-9">
                                {!! Form::select('category_id[]', $selectCategory, $item->id ? $categoryIdArr : '', ['class'=>'form-control', 'required multiple style="min-height: 200px;"']) !!}
                            </div>
                        </div>
                        {!! \App\Helpers\AdminHelper::getTextInput( trans('admin/cms/news.form_title'), 'title', $item->title ?? '', true, '') !!}

                        <div class="form-group row">
                            <label for="example-text-input" class="col-3 col-form-label">@lang('admin/cms/news.form_link_demo')</label>
                            <div class="col-9">
                                {!! Form::text('demo_link', $item->id ? $item->demo_link : '', ['class'=>'form-control', '']) !!}
                            </div>
                        </div>
                        <div class="form-group row">
                            <label for="example-text-input" class="col-3 col-form-label">@lang('admin/cms/news.form_implement_date') </label>
                            <div class="col-9">
                                {!! Form::text('implement_date', $item->id ? $item->implement_date : '', ['class'=>'form-control', '']) !!}
                            </div>
                        </div>

                        {!! \App\Helpers\AdminHelper::getCropImageInput('Image', 'image', $item->image ?? '', false, '', '300,300') !!}
                        {!! \App\Helpers\AdminHelper::getCropImageInput('Image Home', 'image_home', $item->image_home ?? '', false, '', '272,538') !!}
                        {!! \App\Helpers\AdminHelper::getCropImageInput('Image Intro', 'image_intro', $item->image_intro ?? '', false, '', '1200,600') !!}

                        {!! \App\Helpers\AdminHelper::getEditorInput( trans('admin/cms/news.form_introtext'), 'introtext', $item->introtext ?? '', false, '', ['insert_image' => __('common.insert_image'), 'insert_script' => __('common.insert_script')]) !!}
                        ...
                        {!! \App\Helpers\AdminHelper::getEditorInput(trans('admin/cms/news.form_textplus'), 'text_plus', $item->text_plus ?? '', false, '', ['insert_image' => __('common.insert_image'), 'insert_script' => __('common.insert_script')]) !!}
